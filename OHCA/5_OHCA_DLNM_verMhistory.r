...
# define path to save plots
  filter = "filterSFW"
  outcome = "count_disease_mhistory"
  lagNum = "lag7"
...
patient_Mhistory_0=patient[,c(1:67,73)]
patient_Mhistory_1=patient[,c(1:67,74)]
colnames(patient_Mhistory_0)[68]="count_disease_mhistory"
colnames(patient_Mhistory_1)[68]="count_disease_mhistory"

patient_Mhistory_0$mhistory="0"
patient_Mhistory_1$mhistory="1"

patient_mhistory=rbind(patient_Mhistory_0, patient_Mhistory_1)
patient_mhistory$mhistory=factor(patient_mhistory$mhistory)
patient = copy(patient_mhistory)

##### 2. create crossbasis #####
  # NO2
  kno <- equalknots(patient$NO2_mean, nk = nk_to_use)
  klag <- logknots(lag, nk = nk_to_use)
  ns.basis_no <- crossbasis(patient$NO2_mean,argvar=list(knots=kno),group=patient$air_out_idx, arglag=list(knots=klag), lag=lag); #ns.basis_no # nolint
...
##### 3. create day #####
  ...
  patient <- patient[,c("day","PRE_ER_ARREST_DT","air_out_idx",
                      ...
                      "count_disease_mhistory","mhistory")]
  ...
  # table(patient$count_disease_mhistory,patient$year)
...
##### 4. general model (ESF X) #####
  ...
  # 4-1. ns.basis_no
    # fit
    ns_lag_no <- glm(count_disease_mhistory ~ 
                  ns.basis_no
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-2. ns.basis_so
    # fit
    ns_lag_so <- glm(count_disease_mhistory ~ 
                  ns.basis_so
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-3. ns.basis_co
    # fit
    ns_lag_co <- glm(count_disease_mhistory ~ 
                  ns.basis_co
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-4. ns.basis_o3
    # fit
    ns_lag_o3 <- glm(count_disease_mhistory ~ 
                  ns.basis_o3
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-5. ns.basis_pm
    # fit
    ns_lag_pm <- glm(count_disease_mhistory ~ 
                  ns.basis_pm
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-6. ns.basis_temp_mean
    # fit
    ns_lag_temp <- glm(count_disease_mhistory ~ 
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                   #+ ns.basis_temp_max
                  + ns.basis_humi 
  ...
  # 4-7. ns.basis_humi
    # fit
    ns_lag_humi <- glm(count_disease_mhistory ~ 
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
...
##### 5. create patient_esf data & fit model with ESF #####
  ...
  # 5-1. ns.basis_no
    ...
    # fit
    ns_lag3_esf <- glm(count_disease_mhistory ~  
                  ns.basis_no
                  + ns.basis_temp_mean
                  + ns.basis_humi 
    ...
    # table(patient_esf$count_disease_mhistory)
    # mean(patient_esf$count_disease_mhistory)
    # poi_test=rpois(n=sum(patient_esf$count_disease_mhistory),lambda=0.1531)
    # table(poi_test)   
