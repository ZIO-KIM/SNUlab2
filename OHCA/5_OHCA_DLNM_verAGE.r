...
patient_above70=patient[,c(1:67,71)]
patient_under70=patient[,c(1:67,72)]
colnames(patient_above70)[68]="count_disease_age"
colnames(patient_under70)[68]="count_disease_age"

patient_above70$age="up70"
patient_under70$age="down70"

patient_age=rbind(patient_above70,patient_under70)
patient_age$age=factor(patient_age$age)
patient = copy(patient_age)
...
##### 3. create day #####
  ...
  # table(patient$count_disease_age,patient$year)
...
##### 4. general model (ESF X) #####
  ...
  # 4-1. ns.basis_no
    # fit
    ns_lag_no <- glm(count_disease_age ~ 
                  ns.basis_no
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-2. ns.basis_so
    # fit
    ns_lag_so <- glm(count_disease_age ~ 
                  ns.basis_so
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-3. ns.basis_co
    # fit
    ns_lag_co <- glm(count_disease_age ~ 
                  ns.basis_co
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-4. ns.basis_o3
    # fit
    ns_lag_o3 <- glm(count_disease_age ~ 
                  ns.basis_o3
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-5. ns.basis_pm
    # fit
    ns_lag_pm <- glm(count_disease_age ~ 
                  ns.basis_pm
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
  ...
  # 4-6. ns.basis_temp_mean
    # fit
    ns_lag_temp <- glm(count_disease_age ~ 
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                   #+ ns.basis_temp_max
                  + ns.basis_humi 
  ...
  # 4-7. ns.basis_humi
    # fit
    ns_lag_humi <- glm(count_disease_age ~ 
                  # + ns.basis_temp_tc
                  + ns.basis_temp_mean
                  + ns.basis_humi 
...
##### 5. create patient_esf data & fit model with ESF #####
  ...
  # 5-1. ns.basis_no
    ...
    # fit
    ns_lag3_esf <- glm(count_disease_age ~  
                  ns.basis_no
                  + ns.basis_temp_mean
                  + ns.basis_humi 
    ...
    # table(patient_esf$count_disease_age)
    # mean(patient_esf$count_disease_age)
    # poi_test=rpois(n=sum(patient_esf$count_disease_age),lambda=0.1531)
    # table(poi_test)   
